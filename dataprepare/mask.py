...
def create_mask1(image_path):
    pic_path = image_path
    mask_path = "./images/blue-mask.png"
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path, show, model).mask()
def create_mask2(image_path):
    pic_path = image_path
    mask_path = "./images/black-mask.png"
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path, show, model).mask()
def create_mask3(image_path):
    pic_path = image_path
    mask_path = "./images/red-mask.png"
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path, show, model).mask()
def create_mask4(image_path):
    pic_path = image_path
...
class FaceMasker:
    ...
    def _mask_face(self, face_landmark: dict):
        ...
        for j in range(height2):
            for i in range(width2):
                if mask_img_binary[j, i][0] != 0 or mask_img_binary[j, i][1] != 0 or mask_img_binary[j, i][2] != 0:
                    mask_img_binary[j, i] = (255, 255, 255, 255)
    ...
    def _save(self):
        ...
        print(f'Save to {new_face_path2}')
...
if __name__ == '__main__':
    ...
    for i in range(1,151):
        ...
        if len(i)==1:
            ...
            i= '0'+i
        image_path = f'./without_mask/000{i}.jpg'
        #create_mask1(image_path)
        #create_mask2(image_path)
        create_mask3(image_path)
        #create_mask4(image_path)
